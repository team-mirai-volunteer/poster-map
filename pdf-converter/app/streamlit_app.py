...
- 例2：この画像の左側に表が書かれています。掲示番号の列にN～Mのように番号が、各マスには名称が、その右に住所が書いてあります。これらをリスト化して、「番号」「住所」「説明」をkeyとしたcsv形式で出力してください。必ずcsvだけを出力してください。番号は「N-M」のようにして「番号」へ入れてください。番号は必ず英数小文字にしてください。
""")
