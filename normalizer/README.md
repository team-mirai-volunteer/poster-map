...
2. **逆引きチェックモード**: 
   - Google APIで取得した座標を逆ジオコーディングで検証
   - オプションでJageocoder APIも逆引き検証に使用
   - 元の住所と逆引き結果の住所を比較
...
- **逆引きチェックモード**: 「Jageocoder APIも逆引きに使う」のオン・オフに関わらず、「逆引き不一致時に採用するAPI」でJageocoderを選択可能です
